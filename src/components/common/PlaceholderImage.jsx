...
export default function PlaceholderImage({ type = "illustration", height = "h-72", className = "" }) {
          ...
          <div className="flex-1 rounded-[22px] bg-white border border-gray-100 p-3 overflow-hidden flex flex-col text-left text-gray-700 font-sans">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto py-1">
              {[
                ...
                <div key={index} className={`p-2 rounded-lg border text-[9px] ${item.checked ? "bg-[var(--color-lightcyan)]/50 border-[var(--color-lightcyan)]" : "bg-white border-gray-100"
                  ...
                  <span className={`h-4.5 w-4.5 rounded-full flex items-center justify-center ${item.checked ? "bg-[var(--color-primary)] text-[var(--color-accent)]" : "bg-gray-50 border border-gray-200 text-gray-300"
                    }`}>
          ...
          {[
            ...
            <div key={index} className="flex gap-3 text-[11px] leading-relaxed">
              ...
              <div className="flex-1 pb-3 border-b border-gray-50 last:border-b-0">
                ...
                <span className={`inline-block mt-1 text-[9.5px] font-semibold px-2 py-0.5 rounded ${log.isAlert ? "bg-red-50 text-[var(--color-danger)]" : "bg-gray-50 text-gray-500"
                  }`}>
                  {log.badge}
                </span>
              </div>
  ...
  if (type === "checkPoint") {
    return (
      <div>
        <img src="https://res.cloudinary.com/dkulkn9be/image/upload/v1783242572/Walk_Routs_rdvhlx.png" alt="checkPoint" />
      </div>
    );
  }
  ...
  if (type === "incidentLog") {
    return (
      <div>
        <img src="https://res.cloudinary.com/dkulkn9be/image/upload/v1783242572/Log_Incidents_with_Photo_Video_Proof_1_jv2ml4.png" alt="incidentLog" />
      </div>
    );
  }
